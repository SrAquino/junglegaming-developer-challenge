...
test('traps focus inside paused Options and restores it to Resume', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: 'Play' }).click()
  await page.getByRole('button', { name: 'Pause match' }).click()
  const paused = page.getByRole('dialog', { name: 'Match paused' })
  await paused.getByRole('button', { name: 'Options' }).click()
  const options = page.getByRole('dialog', { name: 'Match options' })
  const first = options.getByLabel('Paused game session time')
  const last = options.getByRole('button', { name: 'Back to pause' })
  await last.focus()
  await page.keyboard.press('Tab')
  ...
  await last.click()
  await expect(paused.getByRole('button', { name: 'Resume match' })).toBeFocused()
})

test('pauses on a hidden tab and freezes elapsed time and active effects', async ({ page }) => {
  await page.clock.install()
  await page.goto('/')
  await page.getByRole('button', { name: 'Play' }).click()
  const arena = page.getByRole('img', { name: 'Pirate Battle arena' })
  await expect(arena).toHaveAttribute('aria-busy', 'false')
  await expect(arena).toHaveAttribute('data-match-id', /.+/)
  await expect(arena).toHaveAttribute('data-elapsed-ms', /.+/)
  await page.keyboard.down('f')
  await page.clock.runFor(100)
  await page.keyboard.up('f')
  await expect.poll(async () => Number(await arena.getAttribute('data-effect-count'))).toBeGreaterThan(0)
  await page.evaluate(() => {
    Object.defineProperty(document, 'hidden', { configurable: true, get: () => true })
    document.dispatchEvent(new Event('visibilitychange'))
  })
  await expect(page.getByRole('dialog', { name: 'Match paused' })).toBeVisible()
  const elapsed = await arena.getAttribute('data-elapsed-ms')
  const effects = await arena.getAttribute('data-effect-count')
  await page.clock.runFor(500)
  expect(await arena.getAttribute('data-elapsed-ms')).toBe(elapsed)
  expect(await arena.getAttribute('data-effect-count')).toBe(effects)
})
...
async function expectArenaToFit(page: import('@playwright/test').Page): Promise<void> {
  ...
  expect(bounds.world.width / bounds.world.height).toBeCloseTo(1600 / 896, 2)
