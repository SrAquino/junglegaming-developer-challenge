...
interface MainMenuScreenProps { onPlay: () => void; onOptions: () => void; onLastResult: () => void; hasLastResult: boolean; configurationKey: string; playerId: string; options: GameOptions }

export function MainMenuScreen({ onPlay, onOptions, onLastResult, hasLastResult, configurationKey, playerId, options }: MainMenuScreenProps) {
  ...
  return (
    <main className="application-shell">
      <section aria-labelledby="game-title" className="panel main-menu">
        ...
        <section className="logbook" aria-label="Captain's log"><h2>Captain's Log</h2>{tab === 'ranking' ? <div id="ranking-panel" role="tabpanel"><RankingPanel configurationKey={configurationKey} configurationLabel={`${options.sessionDurationSeconds} second battles · ${options.enemySpawnIntervalSeconds} second spawn interval`} key={configurationKey} playerId={playerId} /></div> : <div id="history-panel" role="tabpanel"><MatchHistoryPanel playerId={playerId} /></div>}</section>
