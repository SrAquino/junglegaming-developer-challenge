...
export function MatchHistoryPanel({ playerId }: MatchHistoryPanelProps) {
  ...
  return <section aria-label="Match History">{query.isFetching && <p aria-live="polite">Refreshing match history</p>}<p className="logbook-caption">Your recent battles</p><div aria-label="Completed matches" className="logbook-table history-table" role="table"><div className="logbook-row logbook-heading" role="row"><span role="columnheader">Date</span><span role="columnheader">Points</span><span role="columnheader">Duration</span><span role="columnheader">Result</span></div>{query.data.items.map((record) => <div className="logbook-row" key={record.matchId} role="row"><time dateTime={record.playedAt} role="cell">{formatPlayedAt(record.playedAt)}</time><strong role="cell">{record.score}</strong><time role="cell">{formatDuration(record.activeDurationMs)}</time><span className={record.endReason === 'time-expired' ? 'result-time-up' : 'result-defeated'} role="cell">{record.endReason === 'time-expired' ? 'Time up' : 'Defeated'}</span></div>)}</div><Pagination page={page} pageSize={pageSize} total={query.data.total} onPageChange={setPage} /></section>
}
